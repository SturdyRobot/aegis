//! # aegis-web
//!
//! WebAssembly bindings that run the **real** Aegis ReAct engine
//! ([`sturdy_core::ReActEngine`]) directly in the browser — the same deterministic
...
//! JS usage:
//! ```js
//! import init, { WasmAegisAgent } from "./aegis_web.js";
//! await init();
...
/// A browser-hosted Aegis agent. Thin wrapper that wires the real engine to a
/// scripted reasoner + tool and streams each finalized step out to JS.
#[wasm_bindgen]
pub struct WasmAegisAgent {
    max_steps: u64,
    max_tokens: u64,
}
...
impl WasmAegisAgent {
    ...
    pub fn new(config_json: &str) -> Result<WasmAegisAgent, JsValue> {
    ...
    async fn next_action(&self, task: &Task, trajectory: &Trajectory) -> CoreResult<Decision> {
        let (thought, action, tokens): (&str, Action, u64) = match trajectory.len() {
            ...
            _ => (
                ...
                Action::Finish {
                    answer: format!(
                        "Completed \"{}\" in {} steps — every step ran inside Aegis's \
