...
use kedge_mcp::{HttpTransport, McpClient, McpClientManager, McpServerConfig, McpTransport};
...
async fn manager_routes_calls_and_journals_mcp_events() {
    ...
    let ledger = Arc::new(kedge_ledger::Ledger::in_memory().unwrap());
    let run_id = kedge_core::TaskId::new();

    let mgr = McpClientManager::connect(std::slice::from_ref(&cfg), "kedge-test")
        .await
        .unwrap()
    ...
    match &events[0] {
        kedge_ledger::Event::McpToolExecution {
            server,
            tool,
            output,
            is_error,
