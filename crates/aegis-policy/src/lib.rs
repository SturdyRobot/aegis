...
//! in a full OPA/Rego engine (heavy WASM/C bindings, against Aegis's ethos). A
//! small, native Rust matcher over an `aegis-policy.toml`:
//!
//! ```toml
...
//! This is the portable user-space layer; it complements `aegis-probe` (kernel
...
use sturdy_core::{Budget, Observation, ToolCall, ToolExecutor};
use thiserror::Error;
...
impl Policy {
    ...
    /// Parse and compile a policy from an `aegis-policy.toml` file.
