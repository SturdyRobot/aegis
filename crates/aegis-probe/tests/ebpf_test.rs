...
//! target host: `sudo -E cargo test -p aegis-probe --features ebpf -- --ignored`.

use aegis_probe::{activate, ProbePolicy};
...
/// `aegis-probe-ebpf` object built (see BUILD_EBPF.md). Never runs in default CI.
...
fn unauthorized_execve_is_blocked_and_logged() {
    ...
    let exec_violation = events
        .iter()
        .find(|e| matches!(e.kind, aegis_probe::ViolationKind::Exec))
        .expect("expected an exec violation event");
    aegis_probe::record_violation(&ledger, run_id, exec_violation).unwrap();
