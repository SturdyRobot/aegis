...
The portable half of `kedge-probe` (policy model, `NullProbe`, ledger events)
...
> **Status:** the kernel object (`kedge-probe-ebpf`) and the Linux loader
...
- Rust **nightly** with `rust-src` (pinned by `kedge-probe-ebpf/rust-toolchain.toml`)
  and `bpf-linker`:
  ```sh
...
```sh
cd crates/kedge-probe-ebpf
cargo build --release          # → target/bpfel-unknown-none/release/kedge-probe-ebpf
```

`src/linux.rs` expects the object at `$OUT_DIR/kedge-probe-ebpf.o`; a small
...
sudo -E cargo test -p kedge-probe --features ebpf -- --ignored
```
