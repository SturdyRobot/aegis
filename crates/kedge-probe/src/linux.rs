...
/// The compiled kernel object, produced by building `kedge-probe-ebpf` for
/// `bpfel-unknown-none`. The build script / Makefile places it here.
static EBPF_OBJECT: &[u8] = include_bytes_aligned!(concat!(env!("OUT_DIR"), "/kedge-probe-ebpf.o"));
...
impl AyaProbe {
    ...
    pub fn load(policy: ProbePolicy) -> Result<Self, ProbeError> {
        ...
        tracing::info!("kedge-probe: eBPF LSM enforcement active");
        Ok(AyaProbe {
            _ebpf: ebpf,
            events,
        })
...
/// hooks only enforce against Kedge-supervised processes.
