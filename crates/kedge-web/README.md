...
The **hero demo**: the real Kedge ReAct engine ([`kedge-core`](../kedge-core)),
...
```bash
# from the repo root
wasm-pack build crates/kedge-web --target web --release --out-dir pkg
```

Output lands in `crates/kedge-web/pkg/` (~137 KB `.wasm` + JS glue). `pkg/` is a
build artifact — not committed; regenerate with the command above.

## Run locally

```bash
python3 -m http.server 8231 --directory crates/kedge-web
