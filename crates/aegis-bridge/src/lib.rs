//! # aegis_rt — Python bindings for Aegis
//!
//! `pip install aegis-rt` and use Aegis's Rust engine directly from Python — the
...
/// `sha256` content hash of `source` — the deterministic cache key Aegis uses.
#[pyfunction]
...
fn classify_tool(name: &str) -> (String, Option<String>) {
    match aegis_audit::classify(name) {
        aegis_audit::ToolSafety::ReadOnly => ("read_only".to_string(), None),
        aegis_audit::ToolSafety::Mutating { risk } => {
            ("mutating".to_string(), Some(risk.as_str().to_string()))
        }
    }
}

/// A parsed Aegis policy (blocked tools, PII redaction, budgets).
#[pyclass]
struct Policy {
    inner: aegis_policy::Policy,
}

#[pymethods]
impl Policy {
    /// Parse a policy from `aegis-policy.toml` text.
    #[staticmethod]
    fn from_toml(text: &str) -> PyResult<Self> {
        Ok(Policy {
            inner: aegis_policy::Policy::from_toml_str(text).map_err(value_err)?,
        })
    }
...
fn audit_report(
    ...
    let report = aegis_audit::AuditReport::from_ledger(db_path, price_per_1k, runs_per_day)
        .map_err(value_err)?;
    Ok(report.to_json())
}

/// The `aegis_rt` Python module.
#[pymodule]
fn aegis_rt(m: &Bound<'_, PyModule>) -> PyResult<()> {
